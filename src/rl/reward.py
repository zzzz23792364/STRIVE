...
class RLReward(nn.Module):
    ...
    def forward(self, future_pred, tgt_traj, z, prior_out):
        ...
        if (
            ...
            # sum of all distances (strongest gradient signal through 12-step decoder)
            crash_loss = dist_traj.mean()
            loss = loss + self.loss_weights["adv_crash"] * crash_loss
            with torch.no_grad():
                info["crash_dist"] = dist_traj.min().item()
