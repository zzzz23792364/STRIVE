...
def compute_adv_success(future_pred, scene_graph, model, attack_agt_idx):
    ...
    ego_idx = 0
    traj = model.get_normalizer().unnormalize(future_pred).detach()
    lw = model.get_att_normalizer().unnormalize(scene_graph.lw).detach()
...
def main():
    ...
    parser.add_argument("--rl_algo", type=str, default="ppo", choices=["reinforce", "ppo", "direct"])
    parser.add_argument("--num_episodes", type=int, default=1000)
    parser.add_argument("--warmup_steps", type=int, default=50)
    parser.add_argument("--num_iters", type=int, default=300, help="optim iters for direct gradient")
    ...
    parser.add_argument("--w_adv_crash", type=float, default=2.0)
    parser.add_argument("--w_motion_prior", type=float, default=1.0)
    parser.add_argument("--w_coll_veh", type=float, default=20.0)
    parser.add_argument("--w_coll_env", type=float, default=20.0)
    ...
    loss_weights = {
        "adv_crash": cfg.w_adv_crash,
        "motion_prior": cfg.w_motion_prior,
        "coll_veh": cfg.w_coll_veh,
        "coll_env": cfg.w_coll_env,
    }
    reward_fn = RLReward(
        loss_weights,
    ...
    # Direct gradient training: deterministic policy trained with gradients through decoder
    if cfg.rl_algo == "direct":
        model.train()
        Logger.log(f"Direct gradient training ({cfg.num_iters} iters)...")
        z_ego = embed_info["prior_out"][0][ego_mask].detach()
        prior_other = (
            embed_info["prior_out"][0][~ego_mask].detach(),
            embed_info["prior_out"][1][~ego_mask].detach(),
        )
        import tqdm
        for it in range(cfg.num_iters):
            optimizer.zero_grad()
            mu, _ = policy(obs_full[~ego_mask])
            z_full = torch.cat([z_ego, mu], dim=0)
            ...
            if it % 50 == 0 or it == cfg.num_iters - 1:
                ...
                msg = f"It {it:4d} | R={reward.item():+.1f} | avgR={recent_r:+.1f} | SR={recent_sr:.2f}"
        ...
        final_sr = np.mean(episode_successes[-100:])
        Logger.log("=" * 60)
        Logger.log(f"Phase 1 Results (DIRECT, {cfg.num_iters} iters)")
        Logger.log(f"  Best reward: {best_reward:+.1f}")
        Logger.log(f"  Final success rate: {final_sr:.3f}")
        Logger.log("=" * 60)
